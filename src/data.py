...
from .tools import gen_dx_bx, get_lidar_data, img_transform, normalize_img
...
class NoAugmentationSegmentationData(SegmentationData):
    def __init__(self, *args, **kwargs):
        kwargs["is_train"] = False
        super(NoAugmentationSegmentationData, self).__init__(*args, **kwargs)
    ...
    def get_scenes(self):
        ...
        scenes = [scene for split in splits for scene in create_splits_scenes()[split]]
...
def compile_data(
    ...
    nusc = NuScenes(version="v1.0-{}".format(version), dataroot=dataroot, verbose=False)
    parser = {
        "vizdata": VizData,
        "segmentationdata": SegmentationData,
        "noaugsegmentationdata": NoAugmentationSegmentationData,
    }[parser_name]
    traindata = parser(
